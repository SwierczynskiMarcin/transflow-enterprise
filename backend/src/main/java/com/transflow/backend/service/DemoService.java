...
import com.transflow.backend.repository.*;
...
public class DemoService {
    ...
    private static final Object[][] EU_CAPITALS = {
            {"Warszawa", 52.2297, 21.0122, "BASE"},
            {"Berlin", 52.5200, 13.4050, "PORT"},
            {"Paryż", 48.8566, 2.3522, "WAREHOUSE"},
            {"Madryt", 40.4168, -3.7038, "WAREHOUSE"},
            {"Rzym", 41.9028, 12.4964, "PORT"},
            {"Praga", 50.0755, 14.4378, "BASE"},
            {"Wiedeń", 48.2082, 16.3738, "WAREHOUSE"},
            {"Budapeszt", 47.4979, 19.0402, "WAREHOUSE"},
            {"Bratysława", 48.1486, 17.1077, "WAREHOUSE"},
            {"Amsterdam", 52.3676, 4.9041, "PORT"},
            {"Bruksela", 50.8503, 4.3517, "WAREHOUSE"},
            {"Sztokholm", 59.3293, 18.0686, "PORT"},
            {"Oslo", 59.9139, 10.7522, "PORT"},
            {"Helsinki", 60.1695, 24.9354, "PORT"},
            {"Kopenhaga", 55.6761, 12.5683, "PORT"},
            {"Lizbona", 38.7223, -9.1393, "PORT"},
            {"Dublin", 53.3498, -6.2603, "PORT"},
            {"Ateny", 37.9838, 23.7275, "PORT"},
            {"Sofia", 42.6977, 23.3219, "WAREHOUSE"},
            {"Bukareszt", 44.4268, 26.1025, "WAREHOUSE"},
            {"Zagrzeb", 45.8150, 15.9819, "WAREHOUSE"},
            {"Lublana", 46.0569, 14.5058, "WAREHOUSE"},
            {"Wilno", 54.6872, 25.2797, "BASE"},
            {"Ryga", 56.9496, 24.1052, "PORT"},
    ...
    public void autoDispatch(int count) {
        ...
        for (Vehicle vehicle : availableVehicles) {
            ...
            if (approachRoute != null && transitRoute != null) {
                ...
                req.setStartLocationId(startLoc.getId());
                req.setEndLocationId(endLoc.getId());
                req.setRoutePolylineApproaching(approachRoute.polyline());
                req.setRouteDistanceApproaching(approachRoute.distance());
                req.setRoutePolylineTransit(transitRoute.polyline());
                req.setRouteDistanceTransit(transitRoute.distance());
