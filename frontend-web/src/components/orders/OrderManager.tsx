...
export default function OrderManager() {
    ...
    const getStatusBadge = (status: string, progress: number) => {
        ...
        if (status === 'WAITING_FOR_CARGO_CLEARANCE') return <span className="bg-fuchsia-500/20 text-fuchsia-400 px-3 py-1 rounded-full text-xs font-bold uppercase animate-pulse">MSU czeka na przeładunek</span>;
                                ...
                                <td className="p-4">
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <div className="w-full bg-slate-700 rounded-full h-1.5 overflow-hidden">
                                            <div className={`h-full transition-all duration-[2000ms] linear ${order.computedStatus === 'BROKEN' ? 'bg-rose-500' : order.computedStatus === 'RESCUE_APPROACHING' ? 'bg-indigo-500' : order.computedStatus === 'HANDOVER' || order.computedStatus === 'WAITING_FOR_CARGO_CLEARANCE' ? 'bg-fuchsia-500' : order.computedStatus === 'TOW_APPROACHING' || order.computedStatus === 'TOWING' ? 'bg-orange-500' : order.computedStatus === 'APPROACHING' ? 'bg-amber-400' : order.computedStatus === 'LOADING' ? 'bg-blue-400' : 'bg-cyan-400'}`} style={{ width: order.computedStatus === 'LOADING' || order.computedStatus === 'HANDOVER' || order.computedStatus === 'WAITING_FOR_CARGO_CLEARANCE' ? '100%' : `${order.computedProgress * 100}%` }}></div>
                                        </div>
                                    </div>
                                </td>
                            </tr>
