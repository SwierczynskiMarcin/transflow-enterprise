...
import { Truck, Plus, Trash2, Edit2, X, User } from 'lucide-react';
...
interface VehicleDB {
    ...
    driverName?: string; // Dodane pole na imię i nazwisko
}
...
export default function VehicleManager() {
    ...
    const [lat, setLat] = useState(52.0);
    const [lng, setLng] = useState(19.0);

    const loadVehicles = async () => {
        // Pobieramy RÓWNOCZEŚNIE pojazdy i kierowców, żeby ich połączyć
        ...
        // Tworzymy mapę kierowców po ID przypisanego auta
        ...
        // Doklejamy nazwisko kierowcy do każdego pojazdu
    ...
    const resetForm = () => {
        setPlate(''); setBrand(''); setModel(''); setConsumption(25); setCapacity(600); setLat(52.0); setLng(19.0);
        setEditingId(null);
        setIsFormOpen(false);
    };
        ...
        <div className="p-8 h-full w-full overflow-y-auto bg-slate-900 text-slate-200">
            ...
            {isFormOpen && (
                <form onSubmit={handleSubmit} className="bg-slate-800 p-6 rounded-2xl border border-slate-700 mb-8 shadow-xl grid grid-cols-1 md:grid-cols-3 gap-4 border-l-4 border-l-cyan-400">
                    ...
                    <div><label className="block text-xs text-slate-400 uppercase mb-1">Spalanie</label><input type="number" required value={consumption} onChange={e => setConsumption(Number(e.target.value))} className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-white outline-none focus:border-cyan-400" /></div>
                    <div><label className="block text-xs text-slate-400 uppercase mb-1">Pojemność Baku</label><input type="number" required value={capacity} onChange={e => setCapacity(Number(e.target.value))} className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-white outline-none focus:border-cyan-400" /></div>
                    <div className="flex gap-2">
                        <div className="flex-1"><label className="block text-xs text-slate-400 uppercase mb-1">Lat</label><input type="number" step="0.0001" required value={lat} onChange={e => setLat(Number(e.target.value))} className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-white outline-none" disabled={!!editingId} /></div>
                        <div className="flex-1"><label className="block text-xs text-slate-400 uppercase mb-1">Lng</label><input type="number" step="0.0001" required value={lng} onChange={e => setLng(Number(e.target.value))} className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-white outline-none" disabled={!!editingId} /></div>
                    </div>
            ...
            <div className="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden shadow-xl">
                <table className="w-full text-left border-collapse">
                    ...
                    <tr className="bg-slate-900/50 text-slate-400 text-sm uppercase tracking-wider">
                        ...
                        <th className="p-4 border-b border-slate-700">Kierowca</th> {/* NOWA KOLUMNA */}
